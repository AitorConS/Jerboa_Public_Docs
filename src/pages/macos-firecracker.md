...
[distribution pipeline](/macos-distribution/).
...
- Jerboa provides snapshot CLI commands and RPCs for RAM, CPU/device state and
  ...
  [snapshot requirements and acceptance](/macos-snapshots/).
...
- Stop requests a VirtIO power-button shutdown. The updated Nanos kernel sends
  ...
  application cleanup. See [shutdown behavior](/macos-shutdown/).
...
- Native Firecracker statistics aggregate the live supervisor/VMM process
  ...
  cgo; see [statistics semantics](/macos-stats/).
...
[ARM64 PostgreSQL fixture](/postgres-arm64/) now builds a pinned
